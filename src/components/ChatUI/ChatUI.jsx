...
const ChatUI = () => {
  ...
  return (
    <>
      {open && (
        <div className="fixed z-[9999] max-w-[310px] w-full shadow-2xl bottom-[0.6rem] right-[5.9rem]" style={{ transform: "translate(-0%,-0%)" }}>
          <div className="rounded-[6px] bg-[#ffffff]">
            <div className="flex justify-between bg-[#5677e1] items-center p-2 rounded-t-[6px]">
              <div className="flex items-center gap-4">
                <div className="flex relative">
                  <Image width={24} height={24} src="/Image/icon/chatbot.svg" alt="chatbot" />
                  <GoDotFill className="text-[#41eb41] absolute" style={{ top: "90%", left: "105%", transform: "translate(-50%,-50%)" }} />
                </div>
            ...
            <div ref={chatContainerRef} className="bg-[#ffffff] p-2 h-[350px] overflow-y-scroll flex flex-col gap-2 mb-1">
              {messages.map((msg, index) => (
                <div key={index} className={`flex items-center gap-2 ${msg.type === 'user' ? 'justify-end' : ''}`}>
                  <div className={`w-[24px] ${msg.type === 'user' ? 'hidden' : ''}`}>
                    <Image className="border-2 border-[#5677e1] rounded-full" width={24} height={24} src="/Image/icon/chatbot.svg" alt="chatbot" />
                  </div>
                  <div className={`max-w-[220px] text-[12px] ${msg.type === 'user' ? 'text-[#fff] bg-[#5677e1]' : 'text-[#000] bg-[#dce0f2]'} px-2 pt-2 pb-1 rounded-[6px]`}>
                    <p>{msg.info}</p>
                    <span className="flex justify-end text-[10px] mt-1">{msg.time}</span>
                  </div>
                </div>
              ))}
              {loading && (
