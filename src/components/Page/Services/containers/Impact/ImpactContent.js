    ...
    {content.data.map((item, index) => {
      return (
        <div key={index} className="testominal max-w-44 md:max-w-64 py-2 md:p-1">
          ...
          <div className="mb-4 text-2xl md:text-4xl font-semibold flex gap-1 items-end"><div className="text-[40px]"><span className="text-[32px]  md:text-[64px]" > {item.ratio}</span></div>%</div>
          <span className="font-medium text-[24px] capitalize mr-1">{item.status}</span>
